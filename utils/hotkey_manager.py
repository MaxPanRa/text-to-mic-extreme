...
class HotkeyManager:
    ...
    def register_hotkeys(self, hotkey_settings):
        ...
        try:
            ...
            if "focus_writer" in hotkey_settings:
                self.hotkeys.append(keyboard.add_hotkey(
                    self.format_shortcut(hotkey_settings["focus_writer"]),
                    lambda: self.hotkey_focus_writer_trigger()
                ))
    ...
    def hotkey_focus_writer_trigger(self):
        """Bring the app to the foreground and focus the text box."""
        try:
            self.app.after(0, self.app.show_writer_overlay)
        except Exception as e:
            print(f"Error focusing writer window: {e}")
...
5. {focus_writer_shortcut} - Focus Writer
   Brings the app to the front and places the cursor in the text box.
